...
def train_AE(loader, model, optimizer, criterion, scaler, model_identifier, current_epoch):
    ...
    for _batch_idx, (_data, _targets) in enumerate(loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            _pred_rel = _predictions.float() / (_targets.float()+0.0000001)

            _loss = criterion(_pred_rel, torch.ones(
                _pred_rel.shape).to(device))
...
if __name__ == "__main__":
    ...
    print('Starting Trial 1: Prediction Retriever (KVS, MLE)')

    _model_directory = '/beegfs/project/MaMiCo/mamico-ml/ICCS/MD_U-Net/4_ICCS/Results/1_Conv_AE/kvs_50_mle'
    _model_name = 'Model_AE_LR0_0001'
