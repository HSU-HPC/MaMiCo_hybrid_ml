...
class UNET_AE(nn.Module):
    """The UNET_AE class aims at implementing an adapted version of the well-
    known U-Net architecture as presented by Aladdin Persson (YouTube, GitHub).
    The main idea is to first create the DoubleConv class, since this
    functionality is essentially repeated on every level of the U-Net. Next, by
    creating the U-Net class the left (contracting) side, the bottleneck and
    the right (expanding side) are implemented.
    ...
    Attributes:
        ...
        features:
          Object of type List containing integers that correspond to the number
          of kernels applied per convolution
        activation:
          Object of PyTorch type torch.nn containing an activation function
    """
    ...
    def forward(self, x, y=0, skip_connections=0):
        """The forward method acts as a quai-overloaded method in that depending
        on the passed flag 'y', the forward method begins with and returns
        different values. This is necessary to later feed time-series latent
        space predictions back into the model.
...
class Hybrid_MD_RNN_UNET(nn.Module):
    """The Hybrid_MD_RNN_UNET class aims at implementing the U-Net and RNN based
    hybrid model for time-series prediction of MD velocity distributions.
    ...
    Attributes:
        UNET_Model:
          Object of type torch.nn.Module containing a trained U-Net model
        RNN_Model:
