...
class Hybrid_MD_RNN_UNET(nn.Module):
    def __init__(self, device, in_channels=3, out_channels=3, features=[4, 6, 8, 10], activation=nn.ReLU(inplace=True), RNN_in_size=256, RNN_hid_size=1024, RNN_lay=2):
        ...
        self.sequence = torch.zeros(5, 256)
        self.fc = nn.Linear(self.hidden_size, self.input_size)
    ...
    def forward(self, x):
        ...
        self.sequence = tensor_FIFO_pipe(
            self.sequence, torch.reshape(x, (1, 256)), self.device).to(self.device)

        x, _ = self.rnn(torch.reshape(self.sequence, (1, 5, 256)), h0)
...
class Hybrid_MD_GRU_UNET(nn.Module):
    ...
    def forward(self, x):
        ...
        x, _ = self.gru(torch.reshape(self.sequence, (1, 5, 512)), h0)
...
def test():
    ...
    model = Hybrid_MD_RNN_UNET(device=device, in_channels=3, out_channels=3, features=[
              4, 8, 16], activation=nn.ReLU(inplace=True), RNN_in_size=256, RNN_lay=2)
